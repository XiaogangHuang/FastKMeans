...
/*
void AdaptiveKmeansV2PCA::fit_ns(const Matrix<point_coord_type> &data)
{
    init_ns(data);
...
void AdaptiveKmeansV2PCA::init_ns(const Matrix<point_coord_type> &data)
{
    n = data.size();
    d = data[0].size();
    feature_cnt = 0;
    ...
    for (size_t i = 0; i < n; i++)
    {
        point_coord_type temp = innerProduct(data[i], points[i].part_normSquare, points[i].rest_norm, pca_dim);
        points[i].total_normSquare = temp;
    }
    sums.resize(k, std::vector<point_coord_type>(d, 0.0));
    ...
    for (size_t i = 0; i < k; i++)
    {
        if (cluster_count[i] > 0)
        {
            ...
            feature_cnt += d;
            point_coord_type temp = centroid_normSquares[i].total_normSquare;
            centroid_normSquares[i].total_normSquare = innerProduct(centroids[i], centroid_normSquares[i].part_normSquare,
                                                                    centroid_normSquares[i].rest_norm, pca_dim);
...
void AdaptiveKmeansV2PCA::assignPoints_ns(const Matrix<point_coord_type> &data)
{
    for (size_t i = 0; i < n; ++i)
    {
        auto &point = points[i];
        ...
        if (globallower < point.distance)
        {
            ...
            for (size_t gi = 0; gi < numGroups; gi++)
            {
                ...
                for (size_t clust : group_index[point.init_clust][gi])
                {
                    // point_coord_type diff_pc = point.total_norm - centroid_normSquares[clust].total_norm;
                    // if (diff_pc * diff_pc < group_second_nearest)
                    // {
                    //     numDistances++;
                    //     // adist = euclidean_dist(data[i], centroids[clust], point.normSquare_total, centroid_normSquares[clust]);
                    //     point_coord_type adist = dist_comp(data[i], centroids[clust], point,
...
bool AdaptiveKmeansV2PCA::recalculateCentroids_ns()
{
    centroids_history.emplace_back(centroids);
    auto &last_centroids = centroids_history.back();
